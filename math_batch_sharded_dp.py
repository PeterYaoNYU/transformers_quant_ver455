...
def run_worker(rank: int, world_size: int, subset_spec: str, out_dir: str):
    ...
    for fname in os.listdir(INPUT_FOLDER):
        ...
        with open(fpath, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if isinstance(obj, list):
                    ...
                    # key-style lines
                    q = obj.get("question") or obj.get("Problem") or obj.get("problem") or ""
                    a = obj.get("answer")   or obj.get("Solution") or obj.get("solution") or ""

                if q:
                    pairs_file.append((q, a))

        N = len(pairs_file)
        if N == 0:
            continue  # nothing to do for this file

        # ------------- balanced sharding for THIS file -------------
        base = N // world_size
        rem  = N % world_size
        ...
        if size == 0:
            continue  # this rank has no slice for THIS file; move to next file
        ...
        part_path = out_dir_p / f"{fname}_{MODEL_ID.replace('/','_')}_rank{rank}.jsonl"
        ...
        with open(part_path, "w", encoding="utf-8") as fout, torch.no_grad():
            for batch_start in tqdm(range(start, end, BATCH_SIZE), disable=(rank != 0)):
                ...
                questions = [pairs_file[i][0] for i in idxs]
                golds     = [extract_gold(pairs_file[i][1]) for i in idxs]
                ...
                for row, (i, gold, in_len) in enumerate(zip(idxs, golds, input_lens)):
                    ...
                    rec = {
                        "idx": i,
                        "question": pairs_file[i][0],   # <-- tuple (q, a)
                        "pred_raw": text,
                        "pred": pred,
                        "gold": gold,
                        "correct": bool(ok),
...
def merge_parts(world_size: int, out_dir: str):
    ...
    # Find all part files like: "<task>.jsonl_<model_tag>_rank<r>.jsonl"
    part_files = list(out_dir_p.glob(f"*_{model_tag}_rank*.jsonl"))
    if not part_files:
        print("No part files to merge.")
        return

    # Group by task name (prefix before _<model_tag>_rankNN.jsonl)
    task_to_parts: dict[str, list[Path]] = {}
    pat = re.compile(rf"^(?P<task>.+?)_{re.escape(model_tag)}_rank\d+\.jsonl$")
    for p in part_files:
        m = pat.match(p.name)
        if not m:
            continue
        task = m.group("task")  # e.g., "Algebra.jsonl"
        task_to_parts.setdefault(task, []).append(p)

    # Merge each task's parts into one file
    for task, parts in sorted(task_to_parts.items()):
        merged_path = out_dir_p / f"{task}_{model_tag}_merged.jsonl"
        all_recs = []
        for part in parts:
        ...
        if not all_recs:
            print(f"[MERGE] {task}: no records found, skipping.")
            continue

        # Sort, compute accuracy, write merged
        all_recs.sort(key=lambda x: x.get("idx", 0))
        correct = sum(int(bool(r.get("correct"))) for r in all_recs)
        total = len(all_recs)
        with open(merged_path, "w", encoding="utf-8") as f:
            for r in all_recs:
                f.write(json.dumps(r, ensure_ascii=False) + "\n")

        print(f"[MERGE] {task}: Acc {correct/total:.3f} ({correct}/{total}) -> {merged_path}")
...
def main():
    ...
    # Two flags so you can explicitly force on/off regardless of the file default
    parser.add_argument("--use-quant-cache", action="store_true",
                        help="Enable quantized KV cache.")
    parser.add_argument("--no-quant-cache", action="store_true",
                        help="Disable quantized KV cache.")
    args = parser.parse_args()

    # Override the global based on CLI
    global USE_QUANT_CACHE
    if args.use_quant_cache and args.no_quant_cache:
        raise SystemExit("Specify only one of --use-quant-cache or --no-quant-cache.")
    if args.use_quant_cache:
        USE_QUANT_CACHE = True
    if args.no_quant_cache:
