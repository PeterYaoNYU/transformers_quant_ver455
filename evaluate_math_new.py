...
def main():
    p = argparse.ArgumentParser(
        description="Evaluate every *.jsonl in a directory."
    )
    p.add_argument("folder", type=Path,
                   help="Directory that contains prediction *.jsonl files.")
    args = p.parse_args()
    ...
    out_path = folder / "eval_results.csv"
    out_path.write_text("\n".join(result_lines), encoding="utf-8")
    print(f"\n✅  Summary appended to {out_path}")
    
    out_path = folder / "eval_results_best.csv"
